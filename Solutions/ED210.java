...
public class BSTree<T extends Comparable<? super T>> {   
   ...
   public boolean valid(BSTNode<T> n) {
      if (n == null) return true;
      return (allLess(n.getLeft(), n.getValue()) && 
              allGreater(n.getRight(), n.getValue()) && 
              valid(n.getLeft()) &&
              valid(n.getRight()));
   }

   public boolean allLess(BSTNode<T> n, T val) {
      if (n == null) return true;
      if (n.getValue().compareTo(val) > 0) return false;
      return allLess(n.getLeft(), val) && allLess(n.getRight(), val);
   }
   public boolean allGreater(BSTNode<T> n, T val) {
      if (n == null) return true;
      if (n.getValue().compareTo(val) < 0) return false;
      return allGreater(n.getLeft(), val) && allGreater(n.getRight(), val);
   }
   ...
   /* public boolean valid() {
      if (root.getLeft().getValue().compareTo(root.getRight().getValue()) > 0) return false;
      return validLeft(root.getLeft(), root.getValue()) && validRight(root.getRight(), root.getValue());
   }

   public boolean validLeft(BSTNode<T> n, T val) {
      if (n==null) return true;
      if (n.getValue().compareTo(val) > 0 ) return false;

      boolean valLeft = true, valRight = true;
      if (n.getLeft() != null)
         if (n.getLeft().getValue().compareTo(n.getValue()) > 0) valLeft = false;
      if (n.getRight() != null)
         if (n.getRight().getValue().compareTo(n.getValue()) < 0) valRight = false;
      if (!(valRight && valLeft)) return false;
      return validLeft(n.getLeft(),n.getValue()) && validRight(n.getRight(), val);
   }

   public boolean validRight(BSTNode<T> n, T val) {
      if (n==null) return true;
      if (n.getValue().compareTo(val) < 0) return false;
      boolean valLeft = true, valRight = true;
      if (n.getLeft() != null)
      ...
      if (n.getRight() != null)
         if (n.getRight().getValue().compareTo(n.getValue()) < 0) valRight = false;
      if (!(valRight && valLeft)) return false;
      return validLeft(n.getLeft(),val) && validRight(n.getRight(), n.getValue());
   } */
}
